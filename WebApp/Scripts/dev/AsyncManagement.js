...
function getAssessments()
{
    ...
    $.getJSON(url, function (data) {
        ...
        $.each(data, function (key, val) {
            tabl += "<tr>";
            tabl += "<td>" + FormatDate(new Date(val.Date)) + " " + val.Time + "</td>";
...
function getAssessmentsOfPatient(val)
{
    if (val != null)
    {
        $('#assAssessments tbody').empty();

        var url = 'http://localhost:8236/api/AssessmentsApi/GetAssessmentsOfPatient?patid=' + val;
        ...
        $.getJSON(url, function (data) {
            ...
            $.each(data, function (key, val) {
                ...
                tabl += "<td>" + val.Worker + "</td>";
                tabl += "<td>" + val.Type + "</td>";
                tabl += "<td><button type=\"button\" onclick=\"getAssessment('" + val.AssessmentId + "')\" class=\"btn btn-primary btn-xs\" id=\"btnOpenAssessment\">Open</button></td>";
                tabl += "</tr>";
            });
